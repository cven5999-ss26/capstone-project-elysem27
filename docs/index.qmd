...
Standard rates for most categories in Social Institutions are below 5kWh, with the largest usage from NGO and Carpentry.
...
ggplot(daily_total_composition, aes(x = Date, y = Total_kWh_avg, fill = customer_type)) +
  ...
  labs(
    title = "Daily Grid Load Curve and Type Composition",
    subtitle = "Aggregated daily totals highlighting average demand",
    x = "Date",
...
It's also important to understand the peak kWh values to right-size the grid. This comes from the max kWh/h values, taking the maximum load during the highest use point during the day. Let's first group by day, then sum the value per day and see the composition from different types. This will give insights for the peak rated value for the grid (kWp) and potential back-up sources like diesel gen-sets and hydropower partnerships.

```{r}
#| message: false
#| warning: false
#| fig-alt: "A stacked chart showing total daily energy loads across calendar days, filtered to exclude rows with zero peak load. Each bar represents a single day's highest consumption, color-coded by connection type to illustrate contribution to peak load."
...
ggplot(daily_composition, aes(x = Date, y = Max_kWh_peak, fill = customer_type)) +
  geom_area(alpha = 0.8, color = "white", linewidth = 0.1) +
  labs(
    title = "Peak Grid Load Curve and Type Composition",
    subtitle = "Aggregated hourly totals highlighting peak demand periods",
    x = "Date",
